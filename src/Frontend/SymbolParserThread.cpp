...
void SymbolParserThread::addSymbol(std::vector<Symbol*>& symbols, Symbol * sym, std::set<wxString> names)
{
	...
	{
		//保留完整符号列表，在AutoCompletion时需要作简单的语义分析
		//return;
	}
	symbols.push_back(sym);
}

bool isKeyword(const wxString& token)
{
	static char words[] = ",if,else,elseif,then,end,while,do,for,in,inpairs,return,break,true,false,local,";
	char *pos = NULL;
	if ((pos = strstr(words, (',' + token + ',').ToAscii())) != NULL)
	{
		return true;
	}
	return false;
...
void SymbolParserThread::ParseFileSymbols(wxString & fileName, wxInputStream& input, std::vector<Symbol*>& symbols, std::set<wxString> names)
{
    ...
    while (GetToken(input, token, lineNumber))
    {
		if (isKeyword(token))
		{
			addSymbol(symbols, new Symbol("", token, lineNumber, fileName, Symbol::SymbolKeyword), names);
		}
		...
		else if (token == "function")
		{
			...
			Symbol *newSym = NULL;
			wxString lineText;
			PeekCurLine(input, lineText);

            if (t2 == "(")
            {
                // The form function Name (...).
				wxString module = fileModule.IsEmpty() ? "" : fileModule;
				addSymbol(symbols, newSym = new Symbol(module, t1, defLineNumber, fileName, Symbol::SymbolFunction), names);
            }
            else
            {
                wxString name;
                ...
                if (t2 == "." || t2 == ":")
                {
					addSymbol(symbols, newSym = new Symbol(t1, name, defLineNumber, fileName, Symbol::SymbolFunction), names);
                }
            }
			if (newSym)
			{
				lineText.Replace("function ", "");
				lineText.Replace("function", "");
				newSym->func_definiton = lineText;
			}
        }
		else if(isIdentify(token))
		{
			...
			//lua函数或者userData（class）名
			else if (next == "(")
			{
				addSymbol(symbols, new Symbol("", token, lineNumber, fileName, Symbol::SymbolFunctionCall), names);
			}
			else if (next == '=' && isIdentify(next2))
			{
				addSymbol(symbols, new Symbol("", token, lineNumber, fileName, Symbol::SymbolReturnIdentifier), names);
			}
			else
			{
				//普通字符串
