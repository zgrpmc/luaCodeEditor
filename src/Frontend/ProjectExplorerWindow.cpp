...
void ProjectExplorerWindow::AddFile(wxTreeItemId parent, Project::File* file)
{
	...
	wxTreeItemId child = m_tree->GetFirstChild(parent, value);
	wxTreeItemId findItem;
	while (child.IsOk())
	{
		ItemData *p = (ItemData*)m_tree->GetItemData(child);
		if (p != NULL && p->file == file)
		{
			findOK = true;
			findItem = child;
			break;
	...
	wxTreeItemId fileNode;
	if (findItem.IsOk())
	{
		m_tree->DeleteChildren(findItem);
		fileNode = findItem;
	}
	else
	{
		fileNode = m_tree->AppendItem(parent, file->GetDisplayName(), image, image, data);
	}
	...
	if (gMainFrame->GetSelectedPageInfo().file==file)
		m_tree->Expand(fileNode);
}
...
void ProjectExplorerWindow::AddSymbol(wxTreeItemId parent, Project::File* file, Symbol* symbol)
{
	...
	if (symbol->func_definiton.size() != 0)
		fullName = symbol->func_definiton;
