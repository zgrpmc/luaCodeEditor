...
public:
	...
	void OnEditKeyDown(wxScintillaEvent& event);
	/**
...
private:
	 ...
	 */
	bool GetIsIdentifierChar(unsigned char c) const;
	...
	//是否落在函数的括号内
	bool isInBrace(int position, int& bracePos);

	void replaceLine(int nLine, const wxString& text);
