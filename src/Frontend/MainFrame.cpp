...
BEGIN_EVENT_TABLE(MainFrame, wxFrame)
	...
	EVT_MENU(ID_ToolsReloadAPI, MainFrame::OnToolsReloadAPI)
...
//    m_mgr.GetPane(m_breakpointsWindow).Name("breakpoints");
 ...
 //   m_mgr.GetPane(m_searchWindow).Name("search");
	...
	m_autoCompleteManager.loadAllAPIs();
}
...
void MainFrame::OnToolsReloadAPI(wxCommandEvent& event)
{
	m_autoCompleteManager.loadAllAPIs();
}
...
void MainFrame::UpdateToolsMenu()
{
	...
	m_menuTools->Append(ID_ToolsReloadAPI, _("Reload APIs"));
	m_menuTools->AppendSeparator();
...
OpenFileInfo* MainFrame::OpenProjectFile(Project::File* file)
{
    ...
    else if (file->scriptIndex != -1)
    {
		...
		std::string gbk = str2ansi(script->source.c_str());
        // Set a save point so that the editor doesn't think we need to save this file.
		openFile->edit->SetText(gbk.c_str());
        openFile->edit->SetSavePoint();
        openFile->edit->EmptyUndoBuffer();
...
bool MainFrame::SaveFile(OpenFileInfo* file, bool promptForName)
{
	...
	std::string fileContent(file->edit->GetText(), file->edit->GetTextLength());
	
	if (!saveFile(fullPath, fileContent))
    {
...
OpenFileInfo MainFrame::GetSelectedPageInfo() const
{
    if (m_notebook->GetPageCount() > 0)
    {
        const wxWindow* selection = m_notebook->GetPage(m_notebook->GetSelection());
