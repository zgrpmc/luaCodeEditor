...
bool PeekToken2(wxInputStream& input, wxString& result)
{
	auto pos = input.TellI();
    unsigned int lineNumber = 0;
	GetToken(input, result, lineNumber);
...
bool PeekCurLine(wxInputStream& input, wxString& result)
{
	auto pos = input.TellI();

	wxString before, behind;
	int pos2 = pos;
	while (pos2 > 0 && before.size()==0 )
	{
		int pos1 = pos2;
		if (pos2 > 100)
		{
			input.SeekI(pos2 - 100);
			pos2 = pos2 - 100;
		}
		else
		{
